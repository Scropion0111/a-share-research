...
def render_nav_tabs():
    ...
    完全移除 st.button，使用纯 JavaScript 修改 URL
    """
    tabs = [
        (0, "📊", "信号清单"),
        (1, "📈", "行情视图"),
    ...
    # 获取当前 tab（从 URL 查询参数）
    try:
        current_tab = st.query_params.get("tab", "0")
    ...
    # JavaScript：使用 pushState 修改 URL 查询参数
    js_code = '''
    <script>
    function switchTab(idx) {
        ...
        if (window.parent && window.parent.Streamlit) {
            window.parent.Streamlit.setQueryParams({tab: idx});
        }
    }
    ...
    if current_tab == 0:
        ...
        if not access_key:
            ...
            <div style="text-align:center; padding:16px 0 24px;">
                <strong style="color:#f59e0b; cursor:pointer;" onclick="switchTab(2)">
