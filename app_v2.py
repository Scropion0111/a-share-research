...
def render_nav_tabs():
    ...
    for idx, icon, name in tabs:
        ...
        tabs_html += f'''<div class="nav-btn {active_class}" 
        onclick="document.getElementById('nav_tab_{idx}').click()">
        <span class="nav-icon">{icon}</span>{name}</div>'''
    tabs_html += '</div></div>'
    ...
    st.radio(
        "",
        options=[name for _, icon, name in tabs],
        index=current_tab,
        key="nav_radio",
        label_visibility="collapsed",
        on_change=lambda: st.query_params.update({"tab": str(st.session_state.nav_radio.index)})
    )
...
def main():
    ...
    # 获取当前 tab
    current_tab = int(st.query_params.get("tab", "0"))
    ...
    if current_tab == 0:
        ...
        if not access_key:
            ...
            <div style="text-align:center; padding:16px 0 24px;">
                <strong style="color:#f59e0b; cursor:pointer;" onclick="switchToTab(2)">
