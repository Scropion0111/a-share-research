...
# UI 模块
from ui.navbar import eigenflow_navbar, disable_sidebar
...
# ==================== 页面组件 | 导航（已迁移到 ui/navbar.py） ====================

def render_nav_tabs(active_page: str = 'signals') -> str:
    """
    渲染导航栏 - 已迁移到 ui/navbar.py
    保持兼容性，返回当前页面 key
    
    Args:
        active_page: 当前页面 ('signals' | 'chart' | 'support')
    
    Returns:
        当前选中的页面 key
    """
    return eigenflow_navbar(active_page)
    ...
    if current_page == 'signals':
        ...
        if not access_key:
            ...
            <div style="text-align:center; padding:16px 0 24px;">
                <strong style="color:#f59e0b; cursor:pointer;" onclick="document.getElementById('trigger_tab_2').click()">
